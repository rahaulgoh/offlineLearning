...
SOURCE_TABLE = "raw_humid_record"
STATE_TABLE  = "edge_infer_state"
SCORE_TABLE  = "edge_humid_health_score"
...
MODEL_PATH = "/opt/edge/models/humid_health/model_humid_health.joblib"
META_PATH  = "/opt/edge/models/humid_health/humid_health_metadata.json"
...
def main():
    ...
    if not tags:
        print("[infer_humid_health] No tags found that match metadata baselines. Exiting.")
        return
    ...
    print(
        f"[infer_humid_health] start: tags={len(tags)} window={window_size} stride={stride} poll={POLL_SECONDS}s"
    )
    ...
    while True:
        ...
        if cycle % 30 == 0:
            print(f"[infer_humid_health] cycle={cycle} inserted={inserted}")
