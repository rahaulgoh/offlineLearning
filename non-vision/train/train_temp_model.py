...
OUTPUT_FOLDER = "non-vision/sensor_models/temp"
...
def get_all_tag_ids() -> List[str]:
    query = f"SELECT DISTINCT {TAG_COL} FROM {TABLE_NAME};"
...
def safe_tag_id(tag_id: str) -> str:
    return "".join([c for c in str(tag_id) if c.isalnum() or c in ("-", "_")])
...
def main() -> None:
    ...
    print(f"Found {len(tag_ids)} sensors. Building training set for generic temperature model.")
    ...
    model_path = os.path.join(OUTPUT_FOLDER, "model_temp.pt")
    ...
    meta_path = os.path.join(OUTPUT_FOLDER, "temp_metadata.json")
    with open(meta_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)

    print(f"Saved generic model to {model_path}")
